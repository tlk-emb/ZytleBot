...
#define LEFT_CURVE_END_TIME 2.2
#define LEFT_CURVE_END_MARGIN_TIME 1
#define LEFT_CURVE_VEL 0.15
...
// 曲がるのが早ければ大きく、遅ければ小さく
#define INTERSECTION_PREDICTION_TIME_RATIO 1.1
// カメラの下部から実際に曲がるまでのマージン(s)を決定 TIME_RATIOの影響を受ける
#define INTERSECTION_PREDICTION_UNDER_MARGIN 1.8
...
////////////////関数//////////////////
    ...
    void leftTurn() {
        ...
        /*
        else if (now - phase_start_t > ros::Duration(LEFT_CURVE_END_TIME + LEFT_CURVE_END_MARGIN_TIME)) {
            changePhase("straight");
        }
         */
    }
                ...
                for (int j = 0; j < lines.size(); j++) {
                    ...
                    if (check_line.degree < 20 && check_line.degree > -20) {
                        ...
                        if (dir == 1) { // 右に伸びている場合
                            ...
                            if (lines[i][2] < BIRDSEYE_LENGTH * 1.5) {
    ...
    void updateObject() {
        ...
        for (itr = objects.begin(); itr != objects.end();) {
            ...
            if (obj.objType == "left_lane_left_T") { // 今だけテスト
                ...
                std::cout << now - obj.timeStamp -  ros::Duration(tempTime) << std::endl;
